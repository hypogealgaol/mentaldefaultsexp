...
function rotateAnnotationCropper_shapeRotation(offsetSelector, xCoordinate, yCoordinate, cropper, shape){
        //alert(offsetSelector.left);
        var x = xCoordinate - offsetSelector.offset().left - offsetSelector.width()/2;
        var y = -1*(yCoordinate - offsetSelector.offset().top - offsetSelector.height()/2);
        var theta = Math.atan2(y,x)*(180/Math.PI);        
        ...
        var rotate = 'rotate(' +cssDegs + 'deg)';
        cropper.css({'-moz-transform': rotate, 'transform' : rotate, '-webkit-transform': rotate, '-ms-transform': rotate});
        shape.css({'-moz-transform': rotate, 'transform' : rotate, '-webkit-transform': rotate, '-ms-transform': rotate});
        $('body').on('mouseup', function(event){ $('body').unbind('mousemove')});
        //return cssDegs;
        output =  cssDegs;
        output = (output % 360) + 1;
...
$(document).ready(function(){               
    ...
    $('#marker').on('click', function(event){ // works!
        ...
        if (toggleMove) {$('#marker').css({'background': goColor});

        } else {$('#marker').css({'background': stopColor});};
    });
    ...
    $('body').on('click',function(event){ // works for the first click, but due to overlap doesn't work after shut off.
        if (!$(event.target).closest('#marker').length) {
            // $('#myDiv').hide();
            toggleMove = false;
            $('#marker').css({'background': stopColor});
        };
    });
    $('html').mousemove(function(event){        
        if (toggleMove){                
            // degs = rotateAnnotationCropper_circleExpansion($('#innerSquare').parent(), event.pageX,event.pageY, $('#marker'),expansion);                
            degs = rotateAnnotationCropper_shapeRotation($('#innerCircle').parent(), event.pageX,event.pageY, $('#marker'),$('#testSquare'));                
            }
        }); 
